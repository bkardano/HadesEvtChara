...
if (AnalysisParameter.Contains("apr12")) {
      ...
      //occupancyNewFile    = "/lustre/hades/user/bkardan/weights/flow2024_08_apr12_gen8_pass7_day107-109_PID0_Occupancy.root";
      //sectorFilelist      = "/lustre/hades/dst/apr12/gen8/sector_selection/FileListHadron.list";
      EPresCorrFile       = "/lustre/hades/user/bkardan/weights/EPHistQA_apr12_gen8_pass37_EPResolution_TOFRPC_FWCharge_ParameterFileCal.root";
...
if( EPresCorr ){
    for(Int_t iHarm=0; iHarm<EPres.size(); iHarm++){
        ...
        //cout << "EP : " << iHarm+1 << "  res:" << EPres[iHarm] << "  TOFRPC:" << TOFRPC << "  FW:"<< FWSumChargeZ <<  endl;
    }
}
...
for (int iHarm = 0; iHarm < (int)nHarmomics; ++iHarm) {
   ...
   if(EPres[iHarm] > 0 && EPres[iHarm] < 1.) {
       //if(Vn > EPres[iHarm]) cout << "Vn : " << Vn <<" Harm: "<< iHarm+1 << "  res:" << EPres[iHarm] << "  TOFRPC:" << TOFRPC << "  FW:"<< FWSumChargeZ <<  endl;
       Vn /=EPres[iHarm];
       Sn /=EPres[iHarm];
   }
   //Float_t weightEP = weight;
   //if(EPres[iHarm] > 0 && EPres[iHarm] < 1.) weightEP /= EPres[iHarm];
   //cout << "weightEP : " << weightEP <<" Harm: "<< iHarm+1 << "  res:" << EPres[iHarm] << "  TOFRPC:" << TOFRPC << "  FW:"<< FWSumChargeZ <<  endl;
